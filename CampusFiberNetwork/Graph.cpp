...
/**
 * @brief 图类析构函数
 * @note 释放边数组和MST边数组占用的所有内存资源，防止内存泄漏
 */
Graph::~Graph() {
    // 释放边数组
...
bool Graph::isConnected() const {
    ...
    for (int i = 0; i < edgeCount; ++i) {
        if (edgesArray[i] != nullptr) {
            int u = edgesArray[i]->getSrc();
            int v = edgesArray[i]->getDest();
            uf.unite(u, v);
        }
    }
...
void Graph::kruskalMST() {
    ...
    for (int i = 0; i < edgeCount && mstEdgeCount < vertices - 1; i++) {
        ...
        if (uf.find(u) != uf.find(v)) {
            uf.unite(u, v);
            mstEdges[mstEdgeCount] = new Edge(*currentEdge);
    ...
    if (mstEdgeCount == vertices - 1) {
        ...
        for (int i = 0; i < mstEdgeCount; i++) {
            std::cout << mstEdges[i]->getSrc() << "\t" << mstEdges[i]->getDest()
                << "\t" << mstEdges[i]->getWeight() << std::endl;
        }
